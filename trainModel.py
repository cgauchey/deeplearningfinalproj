...
def run_training():
    ...
    parser.add_argument("-m", "--model", default=constants.DEFAULT_MODEL_SAVE_FOLDER,
    ...
    parser.add_argument("-s", "--save_freq", default=5,
    ...
    if args.verbose:
        print("Loading data...")
    data = dataLoad.load_data(
        args.image_folder, args.label_file, verbose=args.verbose)

    # Make 80-10-10 train/val/test split of dataset
    if args.verbose:
        print("Making train/val/test split...")
    train_dataset, val_dataset, test_dataset = dataLoad.make_train_val_test_split(
        data, args.seed, verbose=args.verbose)
    ...
    if args.verbose:
        print("Testing model...")
    avg_test_loss = modelUtils.test(
        best_model, test_dataset, batch_size=args.batch_size, verbose=args.verbose)

    print("Training loss of best model: {}".format(
        train_losses[best_epoch_num]))
